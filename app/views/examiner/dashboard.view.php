...
<body>
        ...
        <div class="flex flex-col w-3/4 px-5 h-screen overflow-y-scroll">
            ...
            <div class="flex justify-evenly gap-5 mt-5">
                <div class="flex flex-col justify-evenly gap-2 text-white">
                    <?php $this->renderComponent('numberCard', ['title' => 'Assigned Groups', 'value' => sizeof($pageData['groupList']), 'bg' => 'btn-primary-color']) ?>
    ...
    <script>
        ...
        const examinerGroups = <?= json_encode(array_values($pageData['groupList'])); ?>;
        const completedTasksStat = examinerGroups.map(group => completedTasks.filter(task => task['group_id'] == group['group_id']).length);
        const todoTasksStat = examinerGroups.map(group => todoTasks.filter(task => task['group_id'] == group['group_id']).length);
        const inProgressTasksStat = examinerGroups.map(group => inProgressTasks.filter(task => task['group_id'] == group['group_id']).length);
        ...
        const projectCompletionChart = new Chart(projectCompletionChartCtx, {
            ...
            data: {
                labels: examinerGroups.map(group => group['group_id']),
                datasets: [
                    {
                        label: "Completed",
